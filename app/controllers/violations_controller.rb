class ViolationsController < ApplicationController
  ...
  def create
    ...
    # @user_id = User.where(:name => params[:violation][:user_id]).id
    # @violation.user_id = @user_id
    id = 0
    User.all.each do |u|
        if u.name == params[:user]
            id = u.id
        end
    end
    ...
    respond_to do |format|
      if @violation.save
        format.html { redirect_to  violations_url, notice: 'Violation was successfully created.' }
  ...
  private
    ...
    def violation_params
      params.require(:violation).permit(:user_id, :reason)
    end
end
